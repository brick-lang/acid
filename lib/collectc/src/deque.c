...
enum cc_stat deque_add_at(Deque *deque, void *element, size_t index) {
  ...
  if (index == 0) return deque_add_first(deque, element);

  if (index == c) return deque_add_last(deque, element);
...
enum cc_stat deque_remove_at(Deque *deque, size_t index, void **out) {
  ...
  if (index == 0) return deque_remove_first(deque, out);

  if (index == c) return deque_remove_last(deque, out);
...
enum cc_stat deque_filter_mut(Deque *deque, bool (*pred)(const void *)) {
  if (deque_size(deque) == 0) return CC_ERR_OUT_OF_RANGE;
...
enum cc_stat deque_zip_iter_next(DequeZipIter *iter, void **out1, void **out2) {
  ...
  if (d1_last == d1_first || iter->index >= iter->d1->size) return CC_ITER_END;
  ...
  if (d2_last == d2_first || iter->index >= iter->d2->size) return CC_ITER_END;
