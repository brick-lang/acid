...
void counter_inc_ref(counter_t *counter) { counter->ref_count++; }
...
void counter_inc_store(counter_t *counter) { counter->store_count++; }
...
//  debug_snprintf(countstr, COUNTSTRLEN, "COUNT: store=%zu / ref=%zu",
//  atomic_load(&counter->store_count), atomic_load(&counter->ref_count));
//  return countstr;
//}
